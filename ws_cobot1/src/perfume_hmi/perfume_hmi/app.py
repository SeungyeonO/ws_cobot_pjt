...
로그 모니터링, 로봇 정지, 홈 복귀, 그리퍼 수동 개폐, 속도 모드
전환, 키오스크 잠금 토글, 로그 비우기)뿐이다.
(정지는 재개/복구 개념이 없다 — 정지하면 cobot_control이 종료되므로 새 주문은
재실행 후 받는다. Safe-Off 복구는 티칭펜던트로 한다.)
(일시정지/재개는 두산 드라이버가 모션 진행 중에 응답을 못 하는 구조라 제거 — robot_bridge 주석 참고)

로봇과 랜선으로 직접 연결된 PC에서 robot_bridge.py(ROS2)와 같은 프로세스로
띄운다. 키오스크 잠금 상태는 ROS2 래치 토픽(/kiosk_locked, robot_bridge의
KIOSK_LOCK_TOPIC)으로 발행하고 키오스크 백엔드(perfume_kiosk, 다른 PC)가
구독한다 — kiosk_locked의 단일 소스는 이 서버다. (예전의 서버-to-서버
HTTP(GET /internal/lock_status) 방식은 HMI PC IP를 kiosk 쪽 환경변수로
맞춰줘야 해서 DDS 자동 발견을 쓰는 토픽으로 교체했다. 제조 요청은 kiosk가
로봇 제어부 패키지에 직접 보낸다 — perfume_kiosk 쪽 문서 참고)
...
- /admin                    관리자 HMI (PIN 로그인 필요 — 미로그인 시 로그인 페이지 표시)
  ...
  - POST /api/admin/stop     로봇 정지 (/stop_perfume 발행 — 실제 정지(move_stop+시퀀스 중단)는
                             cobot_control이 수행. 소프트웨어 정지라 안전 정지/비상 정지 아님
                             — robot_bridge 주석 참고)
...
# 이 전역이 잠금 상태의 단일 소스. 값이 바뀔 때마다(admin_lock 라우트)
# bridge.publish_kiosk_lock()으로 래치 토픽(/kiosk_locked)에 발행하면
# 키오스크 PC가 그 토픽을 구독해서 읽어간다.
kiosk_locked = False
...
@app.before_request
def _guard_requests():
    """모든 /api/admin/*(login 제외) 요청을 한 곳에서 인증 검사.

    브라우저 세션 쿠키 기반 — admin_login이 session["admin_at"]을 채우고,
    _admin_authed()가 30분 이내인지 매 요청마다 검사한다.
...
def admin_stop():
    """로봇 정지 — /stop_perfume 발행. 실제 정지(move_stop+시퀀스 중단)는 cobot_control 담당.

    소프트웨어 정지라 안전 정지/비상 정지가 아니다 — robot_bridge 주석 참고.
    """
...
def admin_lock():
    """키오스크 잠금/해제 토글 — 바뀐 값을 래치 토픽(/kiosk_locked)으로 발행한다."""
    global kiosk_locked
    data = request.get_json(silent=True) or {}
    kiosk_locked = bool(data.get("locked"))
    bridge.publish_kiosk_lock(kiosk_locked)
