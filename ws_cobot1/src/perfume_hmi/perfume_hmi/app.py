...
에러 모니터링, 비상 정지, 홈 복귀, 그리퍼 수동 개폐, 속도 모드 전환,
키오스크 잠금 토글, 에러 로그 비우기)뿐이다.
...
- /admin                    관리자 HMI (PIN 로그인 필요 — 미로그인 시 로그인 페이지 표시)
  ...
  - GET  /api/admin/status   로봇 연결/조인트/그리퍼/TCP 힘/속도 모드/에러 로그/잠금 상태 (1초 폴링용)
  ...
  - POST /api/admin/clear_errors  에러 로그 비우기
...
def admin_status():
    ...
    이 라우트는 ROS2를 직접 호출하지 않는다 — robot_bridge.get_status()가
    돌려주는 건 백그라운드 스레드가 토픽 구독으로 이미 쌓아둔 값의 스냅샷일
    뿐이라 가볍다 (자세한 원리는 robot_bridge.py의 init_ros/get_status 참고).
    """
    status = robot_bridge.get_status()
...
def admin_gripper():
    ...
    data = request.get_json(silent=True) or {}
    result = robot_bridge.set_gripper(data.get("action"))
    return jsonify(result), 200 if result["status"] == "success" else 502
...
def admin_speed_mode():
    ...
    data = request.get_json(silent=True) or {}
    mode_map = {"normal": robot_bridge.SPEED_MODE_NORMAL,
                "reduced": robot_bridge.SPEED_MODE_REDUCED}
    mode = mode_map.get(data.get("mode"))
    if mode is None:
        return jsonify({"status": "error", "message": "mode는 normal 또는 reduced여야 합니다."}), 400
    result = robot_bridge.set_speed_mode(mode)
    return jsonify(result), 200 if result["status"] == "success" else 502
...
def admin_clear_errors():
    """에러 로그 비우기 (화면 관리용)."""
    return jsonify(robot_bridge.clear_errors())
...
def main():
    ...
    # 메인 스레드와 별개로, init_ros() 내부에서 ROS2 spin용 데몬 스레드가
    # 하나 더 뜬다 (robot_bridge.py의 init_ros 주석 참고).
    robot_bridge.init_ros()
    atexit.register(robot_bridge.shutdown_ros)
